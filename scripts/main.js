...
function showDictionary(category = "") {
    ...
    let categoryWords = category ? dictionary.categories[category] : [].concat(
        ...Object.values(dictionary.categories)
    );

    categoryWords.forEach(entry => {
...
document.getElementById("translateBtn").addEventListener("click", (event) => {
    ...
    let translation = "Palabra no encontrada.";
    outerLoop:
    for (let category in dictionary.categories) {
        for (let entry of dictionary.categories[category]) {
            if ((language === "es" && entry.spanish.toLowerCase() === word.toLowerCase()) ||
                (language === "en" && entry.english.toLowerCase() === word.toLowerCase())) {
                translation = language === "es" ? entry.english : entry.spanish;
                break outerLoop;
            }
        }
    }
...
document.querySelectorAll("input[name='category']").forEach(input => {
    input.addEventListener("change", () => {
        ...
        showDictionary(selectedCategory);
    });
});
...
document.getElementById("addWordForm").addEventListener("submit", (e) => {
    ...
    if (!english || !spanish || !example || !category) {
    ...
    console.log(`Nueva palabra añadida a la categoría "${category}":`, newWord);
